...
const roundNextHalfHour = date => { const result = new Date(date); const partialMinute = result.getSeconds() !== 0 || result.getMilliseconds() !== 0; result.setMinutes(Math.ceil((result.getMinutes() + (partialMinute ? 0.001 : 0)) / 30) * 30, 0, 0); return result }
...
export default function VisitanteForm({ form, onSubmit }) {
  ...
  useEffect(() => {
    if (endDate) return
    let active = true
    queueMicrotask(() => { if (active) { setFecha(''); setHora(''); setAtajo(''); setAviso('') } })
    return () => { active = false }
  }, [endDate])
  const aplicarAtajo = (key, resolver, redondear) => {
    const resolved = resolver(new Date())
    const result = redondear ? roundNextHalfHour(resolved) : resolved
  ...
  return <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-3 md:grid-cols-2">
    ...
    {visitorType === 'temporal' && <fieldset className="space-y-3 rounded-xl border border-slate-200 p-4 md:col-span-2"><legend className="px-1 text-sm font-medium">Vigente hasta (opcional)</legend>
      <div><span id="vigencia-atajos" className="mb-2 block text-sm font-medium">Atajos de vigencia</span><div className="flex flex-wrap gap-2" role="group" aria-labelledby="vigencia-atajos">
        {[['4h', '4 horas', now => { now.setHours(now.getHours() + 4); return now }, true], ['8h', '8 horas', now => { now.setHours(now.getHours() + 8); return now }, true], ['hoy', 'Hasta las 23:30 de hoy', now => { now.setHours(23, 30, 0, 0); return now }, false], ['manana', 'Mañana al mediodía', now => { now.setDate(now.getDate() + 1); now.setHours(12, 0, 0, 0); return now }, false]].map(([key, text, resolver, redondear]) => <button key={key} type="button" aria-pressed={atajo === key} disabled={key === 'hoy' && new Date() >= new Date(`${hoy}T23:30`)} className={`rounded-lg border px-3 py-2 text-sm font-semibold disabled:opacity-40 ${atajo === key ? 'border-[#4696e5] bg-blue-50 text-blue-800 ring-1 ring-[#4696e5]' : 'border-slate-300 bg-white hover:bg-slate-50'}`} onClick={() => aplicarAtajo(key, resolver, redondear)}>{text}</button>)}</div></div>
