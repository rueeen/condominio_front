...
export default function PropietarioSearch({ value, onChange, label = 'Propietario (opcional)', disabled = false }) {
  ...
  return <div className="relative">
    ...
    {value ? <div className="input flex min-h-11 items-center justify-between gap-2 bg-white"><span>{propietarioLabel(value)}</span><button type="button" className="rounded p-1 hover:bg-slate-100" disabled={disabled} aria-label="Quitar propietario seleccionado" onClick={() => { onChange(null); setBuscar('') }}><X size={16}/></button></div> : <div className="relative"><Search className="pointer-events-none absolute left-3 top-3 text-slate-400" size={18}/><input id={id} className="input w-full pl-10" value={buscar} disabled={disabled} autoComplete="off" placeholder="Nombre, torre o departamento" onChange={event => setBuscar(event.target.value)} aria-describedby={`${id}-status`}/></div>}
